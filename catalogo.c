...
/**
 * Acquisisce la query di ricerca e gestisce la selezione del gioco
 * @return Codice gioco selezionato
 */
long ricercaGioco();

/**
 * Gestisce il menu' per l'amministratore inerente a un determinato gioco
 * @param [in] codice Codice del gioco selezionato
 */
void menuAmministratore(long codice);

/**
 * Gestisce il menu' per il visitatore inerente a un determinato gioco
 * @param codice Codice del gioco selezionato
 */
void menuVisitatore(long codice);

/**
 * Acquisisce la modalità di ordinamento del catalogo
 * @return 1 - copie vendute
 * @return 2 - media valutazione
 */
unsigned short scegliModalita();

/**
 * Recupera e mostra tutte le recensioni di un gioco
 * @param codice Codice del gioco selezionato
 */
...
long ricercaGioco() {
	...
	for (unsigned short i = 0; i < num_elementi; i++) {
		...
		for (int j = 0; gioco.generi[j][0] != '\0'; j++) {
			strcat(generi, gioco.generi[j]);
			if (gioco.generi[j + 1][0] != '\0') strcat(generi, ", ");
		}
